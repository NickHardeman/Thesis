...
void VirusBase::constructCircleBase(float $numPoints, float $r) {
	...
	float angle = 360 / $numPoints;
	 ...
	 for (int i = 0; i < $numPoints; i++) {
		 ...
		 addBasePt(len.x * ofRandom(.2, 10.f), len.y * ofRandom(.2, 10.f), len.x, len.y);
	 }
...
void VirusBase::addBasePt(float $x, float $y, float $tarX, float $tarY) {
	...
	bodyPt.loc.set( $tarX, $tarY);
	bodyPt.originPt.set( $tarX, $tarY );
	
	ofxVec2f normalUp(0, 1);
	ofxVec2f locNormal = bodyPt.loc.normalized();
...
//------------------------------------------------------------
void VirusBase::bounceOffEdges(Particle2D &pt, bool bDampedOnCollision) {
