...
public:
	...
	void setRandomVelocities( ofxVec2f $vel, float $offsetX, float $offsetY, float $minMult, float $maxMult);
	void setRandomLocations(float $minMult, float $maxMult);
	
	void bounceOffEdges( float $damping, float $wallPadding );
	void bounceOffEdges(Particle2D &pt, float $damping, float $wallPadding, float $friction);
